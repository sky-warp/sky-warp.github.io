...
Они позволяют тестировать код приложения в рантайме через корутину и атрибут (об этом позже). Условия для написания таких тестов:

- у них должен быть свой asmdef-файл с референсом на `nunit.framework.dll`;
...
Они запускаются только в редакторе и имеют доступ к методам его жизненного цикла и коду, а также к пространствам имён UnityEngine и UnityEditor.

Они работают на колбэке EditorApplication.update, а не на корутине, но у них такие же условия для написания, что и у тестов в Play mode
